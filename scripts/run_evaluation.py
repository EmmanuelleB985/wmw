...
def _run_condition(
    ...
    for i, ex in enumerate(examples):
        ...
        correct = answers_match(pred_answer, ex.gold_answer, options=ex.options if hasattr(ex, "options") else None)
...
def stage_eval(
    ...
    for model_name in models:
        ...
        rerank_n = min(20, len(all_examples))
        rerank_examples = random.sample(all_examples, rerank_n)
        print(f"    Running reranking (n={rerank_n}, k=5)")
        rerank_traces_list = []
        rerank_correct = 0
        for ex in rerank_examples:
            best, _, scores = rerank_traces(ex, config, k=5)
            pred = extract_answer(best)
            if answers_match(pred, ex.gold_answer, options=ex.options if hasattr(ex, "options") else None):
                rerank_correct += 1
            rerank_traces_list.append(best)
        mr.conditions["rerank"] = ConditionResult(
            model=config.name, condition="rerank",
            traces=rerank_traces_list,
            correct=[answers_match(extract_answer(t), ex.gold_answer, options=ex.options if hasattr(ex, "options") else None)
                     for t, ex in zip(rerank_traces_list, rerank_examples)],
        )
        print(f"    → rerank acc={rerank_correct/rerank_n:.1%}")
